...
- **Patch HTTPS `Accept-Encoding: identity` — acople a la versión de `googleapis` (R2/R4, 2026-07-01):** el patch fuerza `Accept-Encoding: identity` en todos los requests salientes para evitar el "Premature close" (`ERR_STREAM_PREMATURE_CLOSE`) que emite node-fetch/gunzip en Node 22 al descomprimir respuestas gzip de Google (rompía refresh de token y uploads a Drive; incidente 23/06/2026). Desde R4 vive en un módulo compartido `src/utils/httpsIdentityPatch.js`, aplicado como primera instrucción de `src/index.js` y `scripts/recoverWindow.js` (antes estaba duplicado por copy-paste). **Acople frágil:** funciona porque `googleapis@144 → gaxios → node-fetch → https.request`. Una versión más nueva de gaxios usa el `fetch` nativo (undici), que NO pasa por `https.request` → el patch dejaría de interceptar y el "Premature close" volvería **en silencio** (sin error en el patch, solo regresión en auth/uploads). **Regla: antes de actualizar `googleapis`/`gaxios`, revalidar que el patch sigue interceptando los requests a Google (o migrar a `fetch` nativo y removerlo — SPEC_ROBUSTEZ R3).** Smoke test tras cualquier bump: verificar en logs que un refresh de token + un upload responden 200 sin `premature`. Belt-and-suspenders: `premature` ya está clasificado como error reintentable en `src/utils/driveRetry.js` (R5).
