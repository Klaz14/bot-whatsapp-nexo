...
- No modificar `patches/Client.js` sin entender el incidente del ready_timeout (04-05/06/2026): ese archivo es un patch crítico que resuelve la incompatibilidad de `whatsapp-web.js` 1.34.7 con WhatsApp Web 2.3000.x. Modificarlo incorrectamente puede dejar el bot "autenticado pero sordo" (sin listener `message`). Ver NEKO_LOG.md 04-05/06/2026 y `CLAUDE.md` sección 11 para el detalle completo del patch v3 y sus iteraciones.
- Si se actualiza `whatsapp-web.js` en `package.json`, comparar diff entre `patches/Client.js` y el nuevo `Client.js` ANTES de aplicar el update. El patch puede quedar desincronizado con el nuevo `Client.js`; en ese caso, evaluar si la nueva versión incorpora el fix upstream o si hay que portear el patch.
- `whatsapp-web.js` es ingeniería inversa no oficial: Meta puede romperla sin aviso, de un día para el otro. El síntoma "autenticado pero sin ready" sin cambios de código propios es señal de update externo de Meta. Ante ese síntoma, sospechar de incompatibilidad de versión antes de buscar bugs internos.
- Deploy cycle para builds con `patches/Client.js` modificado o cualquier cambio que requiera build limpio: (1) pausar bot con Custom Start Command = `tail -f /dev/null`; (2) commit+push (build corre con bot pausado); (3) SSH: `find /data/.wwebjs_auth/ -name "Singleton*" -delete`; (4) despausar (Custom Start Command vacío); (5) smoke test. Nunca ejecutar el cleanup de Singleton con el bot corriendo; puede corromper la sesión de Chromium.
